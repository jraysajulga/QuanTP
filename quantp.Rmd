...
multisample_boxplot = function(df, sampleinfo_df, outfile, fill_leg, user_xlab, user_ylab)
{
  ...
  htmlwidgets::saveWidget(as_widget(p), 
                          file.path(paste(getwd(), '/', gsub("\\.png", "\\.html", outfile), sep="")),
                          selfcontained = TRUE)
  plot(g);
  dev.off();
}
...
htmllines <- readLines(htmloutfile)
htmllines[1] <- paste('<html>\n<head>\n', paste(prescripts, collapse='\n'), '\n</head>\n<body>')
cat(htmllines, file = htmloutfile)
...
test <- getPlotlyLines('Box_TE_all_rep')
print(test$prescripts)
#print(readLines(paste(outdir,'/','Box_TE_all_rep','.html', sep="")))
```

#name <- 'Box_TE_all_rep'
#lines <- readLines(paste(outdir,name,'.html', sep=""))
#head_scripts <- paste(lines[grep('<script>',lines)[1]
#                            :grep('</head>',lines)[1]], sep="\n")
#paste(head_scripts, sep="\n")[1:10]



```{r}
cat(paste(prescripts, collapse='\n'), file='test.txt')
...
```{r}
string <- ('>eltit/<yltolp>eltit<
{ )(noitcnuf(>redlohecalp<
a etaerc esiwrehto ;ti esu neht ,denifed ydaerla si stegdiWLMTH.wodniw fI //  
eht tceffa taht snoitpo tes ot edoc gnidecerp swolla sihT .tcejbo wen //  
.)tsixe yltnerruc enon hguoht( ssecorp noitazilaitini //  
;}{ || stegdiWLMTH.wodniw = stegdiWLMTH.wodniw')
stringi::stri_reverse(string)
```


```{r}
yo <- paste(c('asdf','funny'), collapse='yo')
yo
```

#htmllines <- readLines(htmloutfile)
#htmllines <- htmllines[1:10]
#grepl('head',htmllines)

#htmllines <- readLines(htmloutfile)
#writeLines(htmllines, con="test.txt")
#htmllines[grepl(htmllines, pattern = 'img')] = 'replaced'
#htmllines

#lines <- readLines(paste(outdir,'Box_TE_rep.html', sep=""))
#scripts <- lapply(lines, grep, pattern="<script src", value=TRUE)
#htmloutput <- gsub('script','placeholder',scripts)
#htmloutput
#print(htmloutput[lapply(htmloutput,length)>0])
#writeLines(htmloutput, con="test.txt")

#name <- 'Box_TE_rep'
#scripts <- lapply(lines, grep, pattern="<script src", value=TRUE)
#javascripts <- lapply(lines, grep, pattern="<script type", value=TRUE)
#divs <- data.frame(lapply(lines, grep, pattern="plotly html-widget", value=TRUE))
#row.names(divs) <- name
