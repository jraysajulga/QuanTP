...
singlesample_regression = function(PE_TE_data,htmloutfile, append=TRUE)
{
  ...
  #library(eeptools)
  #library(autoplotly)
  #print(ggplotly(autoplot(regmodel, data = PE_TE_data)[[1]]))
  #plot_label(g, PE_TE_data$PE_ID)
  #print(ggplotly(g[[1]]))
  #print(ggplotly(autoplot(regmodel, label=TRUE)[[2]]))
  #print(ggplotly(autoplot(regmodel, label=TRUE)[[4]]))
  #print(autoplotly(regmodel))
  
  # hello
  # regmodel = lm(PE_abundance~TE_abundance, data=PE_TE_data);
  # lm(formula = Sepal.Length ~ Petal.Width, data = iris)
  #g <- ggplot(PE_TE_data, aes(x = TE_abundance, y = PE_abundance)) +
  #p1<-ggplot(regmodel, aes(.fitted, .resid))+geom_point()
    #1<-p1+stat_smooth(method="loess")+geom_hline(yintercept=0, col="red", linetype="dashed")
    #p1<-p1+xlab("Fitted values")+ylab("Residuals")
   # p1<-p1+ggtitle("Residual vs Fitted Plot")+theme_bw()
  #print(ggplotly(autoplot(regmodel)[[1]]))
  #print(p1)
  g <- autoplot(regmodel, data = PE_TE_data)[[1]] +
  ...
  #g <- ggplot(PE_TE_data, aes(sample = TE_abundance, y = PE_abundance)) + 
  #  stat_qq() +
  #  stat_qq_line()
  #g <- ggplot(regmodel, aes(qqnorm(.stdresid)[[1]], .stdresid))+geom_point(na.rm = TRUE) +
  ...
  #p2<-ggplot(regmodel, aes(qqnorm(.stdresid)[[1]], .stdresid))+geom_point(na.rm = TRUE)
  #  p2<-p2+geom_abline(aes(qqline(.stdresid)))+xlab("Theoretical Quantiles")+ylab("Standardized Residuals")
  #  p2<-p2+ggtitle("Normal Q-Q")+theme_bw()
  #saveWidgetFix(ggplotly(p2), file.path(gsub("\\.png", "\\.html", outplot)))
  
  #saveWidgetFix(ggplotly(p5), file.path(gsub("\\.png", "\\.html", outplot)))
  ...
  g <- ggplot(regmodel, aes(.hat, .cooksd))+geom_point(na.rm=TRUE)+stat_smooth(method="loess", na.rm=TRUE)
  ...
  cat("<tr><td>Mean Cook\'s distance</td><td>",mean(cooksd, na.rm=T),"</td></tr>\n",
    ...
    dev.off();
    #p <- plot_ly(x = PE_TE_data_no_outlier$TE_abundance, y = PE_TE_data_no_outlier$PE_abundance)
    p <- ggplotly(g)
...
singlesample_scatter = function(PE_TE_data, outfile)
{
  ...
  suppressMessages(plot(g));
  # plot(g);
  g <- ggplotly(g, tooltip = c("label", "x", "y"))
  saveWidgetFix(g, file.path(gsub("\\.png", "\\.html", outfile)))
  dev.off();
}
...
multisample_boxplot = function(df, sampleinfo_df, outfile, fill_leg, user_xlab, user_ylab)
{
  ...
  #row.names(tempdf1) <- tempdf1$variable
  #print(tempdf1)
...
cat("<br><br><font size=5><b><a href='PE_TE_logfold_pval.txt' target='_blank'>Download the complete fold change data here</a></b></font><br>\n",
    ...
    if(length(condition_ind)!=1)
        ...
        g <- plot(TE_df_logfold$LogFold, -log10(TE_pval),
             xlab="log2 fold change", ylab=volc_ylab,
             type="n")
