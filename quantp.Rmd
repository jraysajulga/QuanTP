...
singlesample_regression = function(PE_TE_data,htmloutfile, append=TRUE)
{
  ...
  g <- autoplot(regmodel)[[1]] +
    geom_point(aes(text=sprintf("x: %.2f<br>y: %.2f<br>Gene: %s", .fitted, .resid, PE_TE_data$PE_ID)),
               shape = 2)
  ...
  g <- autoplot(regmodel)[[2]] +
    geom_point(aes(text=sprintf("x: %.2f<br>y: %.2f<br>Gene: %s", .qqx, .qqy, PE_TE_data$PE_ID)),
               shape = 2)
  ...
  g <- autoplot(regmodel)[[4]] +
    ...
    geom_point(aes(text=sprintf("x: %.2f<br>y: %.2f<br>Gene: %s", .hat, .stdresid, PE_TE_data$PE_ID)),
               shape = 2) #+
    #stat_function(fun = cd_cont_pos, args = list(level = 0.5, model = regmodel), xlim = c(0, 0.25), lty = 2, colour = "red") +
    #stat_function(fun = cd_cont_neg, args = list(level = 0.5, model = regmodel), xlim = c(0, 0.25), lty = 2, colour = "red") +
...
cat("<br><br><font size=5><b><a href='PE_TE_logfold_pval.txt' target='_blank'>Download the complete fold change data here</a></b></font><br>\n",
    ...
    if(length(condition_ind)!=1)
        ...
        g <- ggplot(PE_df_logfold, aes(x = PE_df_logfold$LogFold, -log10(PE_pval))) +
              xlab("log2 fold change") + ylab("-log10 p-value") + 
              geom_point()
        saveWidgetFix(ggplotly(g), file.path(gsub("\\.png", "\\.html", outplot_PE)))
        ...
        # hello
        # g <- ggplot(TE_df_logfold, aes(x = TE_df_logfold$LogFold, -log10(TE_pval))) +
        #       xlab("log2 fold change") + ylab("-log10 p-value") + 
        #       geom_point()
        #g = ggplot(PE_TE_data, aes(x=TE_abundance, y=PE_abundance, label=PE_ID)) + geom_smooth() + 
      #xlim(min_lim,max_lim) + ylim(min_lim,max_lim) +
      #geom_point(aes(text=sprintf("Gene: %s<br>Transcript Abundance (log fold-change): %.3f<br>Protein Abundance (log fold-change): %.3f",
       #                         PE_ID, TE_abundance, PE_abundance)))
        ...
        saveWidgetFix(p, file.path(gsub("\\.png", "\\.html", outplot_TE)))
